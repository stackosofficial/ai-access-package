...
const initAIAccessPoint = async (env, skyNodeParam, app, runNaturalFunction, runUpdate, upload) => {
    ...
    if (upload) {
        app.post("/natural-request", upload.array("files"), auth_1.protect, (req, res, next) => (0, checkBalance_1.checkBalance)(req, res, next, contractAddress), (req, res, next) => runNaturalFunction(req, res, balanceRunMain).catch(next));
    }
    else {
